...
class TableNewsVS: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: TableNewsVCell.reuseID, for: indexPath) as! TableNewsVCell
        cell.avatarNews.avatarImage.image = newsList[indexPath.row].avatars
        cell.nameUserNews.text = newsList[indexPath.row].name
        cell.nameUserNews.text = newsList[indexPath.row].date
        cell.nameUserNews.font = UIFont.systemFont(ofSize: 12, weight: UIFont.Weight.light)
        cell.nameUserNews.textColor = UIColor.gray.withAlphaComponent(0.5)
